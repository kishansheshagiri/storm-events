...
jQuery(document).ready(function(){
  ...
  jQuery('#event_state_fips').on('change load',function() {
    var url = '/get_counties_dropdown?us_state_id=' + $(this).val()
    $('#event_county_fips').empty()
    $.get(url, function(data) {
      $.each(JSON.parse(data), function(index, item) {
        $('#event_county_fips').append(new Option(item.name, item.fips));
      });
    });
  })
})
...
function initMap() {
  ...
  var uluru = {lat: 38.923283, lng: -101.52};
  map = new google.maps.Map(document.getElementById('storm-map'), {
    zoom: 4,
    center: uluru
  });
}
...
$(document).ready(function() {
    $('#events-table').DataTable({
      "ajax": $('#events-table').data('source'),
      "bSortClasses": false,
      "processing": true,
      "deferRender": true,
    });
    $('#episodes-table').DataTable({
      "ajax": $('#episodes-table').data('source'),
      "pagingType": "full_numbers",
      "bSortClasses": false,
      "processing": true,
      "deferRender": true,
